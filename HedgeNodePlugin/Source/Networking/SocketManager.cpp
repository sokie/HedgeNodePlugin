...
namespace Network
{
	...
	bool SocketManager::Create_UDP(uint16_t *RequestedPort, bool Blocking)
	{
		...
		if (Handle <= 0)
		{
			HedgePrint("Failed to create a UDP socket for port %u", Port);
			return false;
		}
		...
		for (uint8_t i = 0; i < 11; i++)
		{
			...
			if (bind(Handle, (const sockaddr*)&Address, sizeof(sockaddr_in)) < 0)
			{
				...
				if (i == 9)
				{
					HedgePrint("Failed to bind UDP socket for port %u", Port);
					return false;
				}
				else
				{
					HedgePrint("Failed to bind UDP socket for port %u, retrying", Port);
				}
			}
			else
			{	
				// Leave the loop and set the result port.
				HedgePrint("Bound UDP socket for port %u", Port + i);
		...
		{
			HedgePrint("Failed set blocking status on UDP socket for port %u", Port);
			return false;
		}
#elif PLATFORM == PLATFORM_MAC ||  PLATFORM == PLATFORM_UNIX
	...
	bool SocketManager::Create_TCP(class hAddress *Destination, bool Blocking)
	{
		...
		if (Handle <= 0)
		{
			HedgePrint("Failed to create a TCP socket for IP %s", inet_ntoa(*(in_addr *)&Destination->Address));
			return false;
		}
		...
		{
#if PLATFORM == PLATFORM_WINDOWS
			HedgePrint("Failed to connect to IP %s with error %i", inet_ntoa(*(in_addr *)&Destination->Address), WSAGetLastError());
#elif PLATFORM == PLATFORM_MAC ||  PLATFORM == PLATFORM_UNIX
			HedgePrint("Failed to connect to IP %s with error %s", inet_ntoa(*(in_addr *)&Destination->Address), strerror(errno));
#endif
			return false;
		}
		...
		if (ioctlsocket(Handle, FIONBIO, &NonBlocking) != 0)
		{
			HedgePrint("Failed set blocking status on TCP socket for IP %s", inet_ntoa(*(in_addr *)&Destination->Address));
			return false;
		}
#elif PLATFORM == PLATFORM_MAC ||  PLATFORM == PLATFORM_UNIX
		int32_t NonBlocking = Blocking ? 0 : 1;
